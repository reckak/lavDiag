...
#' curves <- prepare_continuous(
#'   fit, length.out = 200, other_latents = "mean",
#'   se = TRUE, se_summary = "median"
#' )
...
prepare_continuous <- function(fit,
  ...
  if (identical(other_latents, "mean")) {
    ...
    # (This happens e.g., when all scores for a latent are non-finite in a group.)
  ...
  for (g in seq_len(n_groups)) {
    ...
    # Scoped write: compute within rows_k per latent (no group-level eta matrix needed)
    ...
    for (j_idx in seq_along(ov_cont)) {
      ...
      for (k_idx in nz_idx) {
        ...
        # restrict computations to the focal block of rows
