    ...
    for (jj in seq_along(ov_ord)) {
      ...
      for (k_idx in nz_idx) {
        ...
        if (identical(other_latents, "mean")) {
          ...
          if (length(other_r)) {
            # compute IDs for the additional loadings and keep only free (>0)
            extra_ids_raw <- sapply(other_r, function(r) get_id_lam(g, j, latent_names[r]))
            keep <- which(extra_ids_raw > 0L)
            if (length(keep)) {
              extra_ids <- extra_ids_raw[keep]
              extra_A <- do.call(cbind, lapply(keep, function(ii) (-B / se_j) * rep(base_eta[ other_r[ii] ], length(rows_k))))
              colnames(extra_A) <- paste0("lam__", j, "__", latent_names[ other_r[keep] ])
            } else {
              extra_A <- NULL
            }
          }
        ...
        # Build gradient matrix A synchronously with cols_all (avoid mismatch)
        # rows = grid rows_k; columns correspond 1:1 to parameter IDs in cols_all
        A_mats <- list()
        A_ids  <- integer(0)

        # ν_j
        if (id_nu > 0L) {
          A_mats[[length(A_mats) + 1L]] <- matrix(d_nu, ncol = 1L)
          A_ids  <- c(A_ids, id_nu)
        }
        # λ_{jk}
        if (id_lamk > 0L) {
          A_mats[[length(A_mats) + 1L]] <- matrix(d_lamk, ncol = 1L)
          A_ids  <- c(A_ids, id_lamk)
        }
        # thresholds τ_{j,*}
        tau_keep <- which(id_tau > 0L)
        if (length(tau_keep)) {
          A_mats[[length(A_mats) + 1L]] <- d_tau[, tau_keep, drop = FALSE]
          A_ids  <- c(A_ids, id_tau[tau_keep])
        }
        # extra loadings (other latents held at baseline = mean)
        if (!is.null(extra_A)) {
          A_mats[[length(A_mats) + 1L]] <- extra_A
          A_ids  <- c(A_ids, extra_ids)
        }

        A_core   <- if (length(A_mats)) do.call(cbind, A_mats) else NULL
        cols_all <- if (length(A_ids)) A_ids[ A_ids > 0L ] else integer(0)

        if (length(cols_all)) {
          # remove any leftover zero IDs just in case
          cols_all <- cols_all[cols_all > 0L]
          if (length(cols_all) && !is.null(A_core) && ncol(A_core) > 0L) {
