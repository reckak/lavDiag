...
#' @param data Optional: data frame (single-group) or a list of data frames (multi-group)
#'   produced by `lavPredict_parallel(fit, return_type = "list" | "data")`, containing
#'   observed variables and factor scores. If `NULL`, it is computed internally.
#' @param info Optional: a result of `model_info(fit)`. If `NULL`, it is computed internally.
...
#'
#' @return A tibble with `.rid` (row ids used by lavaan), `.gid`, `.group`,
#'   factor scores, observed variables, and selected augmentation outputs.
#' @noRd
.augment_ordinal <- function(fit,
  ...
  # -- Row ids used in estimation (case.idx) -----------------------------------
  # Single-group: integer vector; Multi-group: list per group.
  ...
  # -- Final bind: put .rid first, then .gid, .group ---------------------------
  fs_tbl_parts <- vector("list", n_groups)
  for (g in seq_len(n_groups)) {
    fs_g <- tibble::as_tibble(fs_list[[g]])

    # Pick row ids for this group (fall back to sequence if missing/mismatch)
    rid_g <- tryCatch({
      if (is.list(case_idx)) case_idx[[g]] else case_idx
    }, error = function(e) NULL)
    if (is.null(rid_g) || length(rid_g) != nrow(fs_g)) {
      rid_g <- seq_len(nrow(fs_g))
    }

    # Add .rid as the very first column
    fs_g <- tibble::add_column(fs_g, .rid = rid_g, .before = 1)
    # Then add .gid and .group
    fs_g <- tibble::add_column(fs_g, .gid = g, .before = 2)
  ...
  eta_cols <- eta_cols[eta_cols %in% names(fs_tbl)]
  if (length(eta_cols)) {
