...
#' @param line_width Numeric linewidth for model/empirical curves.
#' @param color_points,color_model,color_empirical Colors for points, model curve/ribbon, empirical curve/ribbon.
#' @param alpha_points,alpha_ribbon Alphas for points and ribbons.
...
item_fit_plot_old <- function(
  ...
  if (isTRUE(show_points)) {
    ...
    # Safe numeric jitter (handles ordered factors) with ±3·SD truncation
    if (!is.null(jitter_seed)) {
      old_seed <- .Random.seed
      on.exit({ if (exists("old_seed", inherits = FALSE)) .Random.seed <<- old_seed }, add = TRUE)
      set.seed(jitter_seed)
    }
    if (nrow(pts_long) > 0) {
      y_num <- if (is.numeric(pts_long$y)) pts_long$y else as.numeric(pts_long$y)
      eps   <- stats::rnorm(nrow(pts_long), sd = jitter_sd)
      lim   <- 3 * jitter_sd
  ...
  if (isTRUE(show_points) && !is.null(pts_long) && nrow(pts_long) > 0) {
    p <- p + ggplot2::geom_point(
      data = pts_long,
      ggplot2::aes(x = !!rlang::sym(latent), y = y_jit),
      inherit.aes = FALSE,
      color = color_points,
      alpha = alpha_points,
      size = point_size,
  ...
  if (isTRUE(show_model)) {
    if (isTRUE(ribbons) && any(!is.na(curves$m_lwr)))
      p <- p + ggplot2::geom_ribbon(ggplot2::aes(ymin = m_lwr, ymax = m_upr, fill = "Model"), alpha = alpha_ribbon)
    p <- p + ggplot2::geom_line(ggplot2::aes(y = m_est, color = "Model"), linewidth = line_width)
  }

  if (isTRUE(show_empirical)) {
    if (isTRUE(ribbons) && any(!is.na(curves$e_lwr)))
      p <- p + ggplot2::geom_ribbon(ggplot2::aes(ymin = e_lwr, ymax = e_upr, fill = "Empirical"), alpha = alpha_ribbon)
    p <- p + ggplot2::geom_line(ggplot2::aes(y = e_est, color = "Empirical"), linewidth = line_width)
  }
  ...
  if (isTRUE(show_metrics) && !is.null(mets) && nrow(mets)) {
    ...
    if (".group" %in% names(curves)) {
      xr  <- curves |>
        ...
        dplyr::summarise(x_mid = median(.data$x, na.rm = TRUE), .groups = "drop")
