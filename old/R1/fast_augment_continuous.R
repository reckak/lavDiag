...
#' lavPredict output (observed vars + factor scores [+ FS SEs]), then
...
.augment_continuous <- function(fit,
  ...
  for (g in seq_len(n_groups)) {
    ...
    if (isTRUE(resid)) {
      ...
      resid_mat <- y_obs - yhat_mat
      colnames(resid_mat) <- paste0(prefix_resid, ov_cont)
    }
    ...
    if (need_dm_glob) {
      ...
      for (j_idx in seq_len(J)) {
        ...
        if (!length(ids)) {
          ...
          # Build A matrix row-wise: columns correspond to uniq free ids
          # For lambda-ids: derivative = eta_k; for nu: +1.
          has_lam <- lam_ids > 0L
          if (any(has_lam)) {
            ...
            if (length(sel_idx)) {
              A_lam <- eta_use[, lat_present[sel_idx], drop = FALSE]
              col_ids <- lam_ids[sel_idx]
              # collapse duplicated ids by summing columns in 'uniq' order
              A <- matrix(0, nrow = N, ncol = length(uniq))
              for (ci_ in seq_along(uniq)) {
                id <- uniq[ci_]
                hit <- which(col_ids == id)
                if (length(hit)) A[, ci_ ] <- rowSums(A_lam[, hit, drop = FALSE])
              }
            } else {
              A <- matrix(0, nrow = N, ncol = length(uniq))
            }
          ...
          # --- SAFE VCOV x GRADIENT HANDLING (patch) --------------------------
