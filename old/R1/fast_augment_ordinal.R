#' Augment ordinal SEM data (fast path; base R only)
#'
#' Adds latent linear predictors (y*), expected scores E[Y], per-category probabilities,
#' residuals (obs - E[Y]), and optionally delta-method SEs/CI for E[Y].
#' Works for single- and multi-group lavaan models with ordinal indicators.
#'
#' @noRd
.augment_ordinal <- function(fit,
  ...
  V_full     <- if (need_dm_glob) lavaan::lavInspect(fit, "vcov") else NULL
      ...
      if (need_dm_glob && isTRUE(yhat) && (isTRUE(se_yhat) || isTRUE(ci))) {
        ...
        if (length(keep)) {
          ...
          if (length(lam_keep)) {
            ...
            p_idx <- p_idx + 1L
            M <- d_lam_mat[, sel, drop = FALSE]; colnames(M) <- lam_keep
            G_parts[[p_idx]] <- M
          }
