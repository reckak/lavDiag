...
#' @param se Logical; append delta-method SEs of E[Y] (default TRUE).
...
augment_ordinal <- function(fit,
  ...
  V_full     <- if (ci || se) lavaan::lavInspect(fit, "vcov") else NULL
  ...
  # Make factor-score columns a contiguous block right after .group
  eta_cols <- info$latent_variables
  eta_cols <- eta_cols[eta_cols %in% names(fs_tbl)]  # keep only present columns
  if (length(eta_cols)) {
