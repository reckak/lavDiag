...
#' - `correct_extremes = TRUE` enables post-hoc re-scoring of only flagged rows using a fallback method
#'   (default `EMB`). Flagging is controlled by `extreme_rule`:
#'   * `"auto"` (default): prefer `"z_by_se"` when factor-score SEs are available; otherwise `"mad"`; and finally `"abs"`.
#'   * `"z_by_se"`: flag rows where `abs(FS) / pmax(SE, eps) > extreme_k` for any latent.
#'   * `"mad"`: per-latent robust Z: `abs(FS - median)/ (1.4826*MAD) > extreme_k`.
#'   * `"abs"`: simple absolute threshold `abs(FS) > extreme_k`.
#' - Set `flag_column = TRUE` to add `.fs_corrected = TRUE` on corrected rows.
#'
...
#' @param method lavaan lavPredict method (e.g., "ml", "regression", "EBM"/"EMB").
#' @param correct_extremes logical; if TRUE, re-score only flagged rows via `fallback_method`.
#' @param extreme_rule one of c("auto","abs","z_by_se","mad"). See details above.
#' @param extreme_k numeric; rule-dependent threshold (default 6).
#' @param extreme_eps small numeric to guard division by zero in z-scores (default 1e-8).
#' @param fallback_method character; fallback for corrected rows (default tries "EMB" then "EBM").
#' @param flag_column logical; if TRUE add `.fs_corrected` column.
...
lavPredict_parallel_old <- function(fit,
                                method             = "ml",
                                # --- Extreme-score correction options ---
  ...
  # -- Helper: robust flagging of extreme FS ----------------------------------
  .flag_extreme_rows <- function(df, lv_cols, rule, k, eps, prefix_se) {
    if (!length(lv_cols) || nrow(df) == 0L) return(rep(FALSE, nrow(df)))

    # Auto choose rule based on availability of SE columns
    if (identical(rule, "auto")) {
      se_cols <- paste0(prefix_se, lv_cols)
      have_se <- any(se_cols %in% names(df))
      rule <- if (have_se) "z_by_se" else "mad"
    }

    if (rule == "abs") {
      fl <- Reduce(`|`, lapply(lv_cols, function(nm) abs(df[[nm]]) > k))
      return(ifelse(is.na(fl), FALSE, fl))
    }

    if (rule == "z_by_se") {
      se_mat <- lapply(lv_cols, function(nm) {
        se_nm <- paste0(prefix_se, nm)
        if (se_nm %in% names(df)) as.numeric(df[[se_nm]]) else NA_real_
      })
      z_ok <- Map(function(x, s) abs(x) / pmax(s, eps), df[lv_cols], se_mat)
      z_max <- do.call(pmax, c(z_ok, na.rm = TRUE))
      fl <- is.finite(z_max) & (z_max > k)
      fl[is.na(fl)] <- FALSE
      return(fl)
    }

    # rule == "mad"
    fl_list <- vector("list", length(lv_cols))
    for (i in seq_along(lv_cols)) {
      x <- as.numeric(df[[lv_cols[i]]])
      med <- stats::median(x[is.finite(x)], na.rm = TRUE)
      mad <- stats::mad(x[is.finite(x)], constant = 1.4826, na.rm = TRUE)
      if (!is.finite(mad) || mad <= 0) {
        fl_list[[i]] <- abs(x - med) > k  # fallback to abs around median
      } else {
        z <- abs(x - med) / mad
        fl_list[[i]] <- z > k
      }
  ...
  if (!has_ord || workers <= 1L) {
    ...
    # --- Post-hoc correction for extreme FS (serial path) ---------------------
    out <- (function(out_in) {
      ...
      flag <- .flag_extreme_rows(out_in, lv_cols, extreme_rule, extreme_k, extreme_eps, prefix_se_fs)
      if (!any(flag)) return(out_in)

      # Build a tiny dataset consisting of flagged rows + stability dummies (per group & category)
      sub_df <- dat_original[flag, , drop = FALSE]
      sub_df$.ix <- seq_len(nrow(sub_df))

      dummy_ord  <- create_dummy_from_original(dat_original, ov_ord, group_var = if (is_mg) gvar else NULL)
      ...
      missing_cols <- setdiff(names(sub_df), names(dummy)); for (cc in missing_cols) dummy[[cc]] <- NA
      ...
      for (mth in try_methods) {
        ...
        pred_ok <- try(suppressWarnings(do.call(lavaan::lavPredict, args_f)), silent = TRUE)
        if (!inherits(pred_ok, "try-error")) break else last_err <- attr(pred_ok, "condition")
      }
      if (inherits(pred_ok, "try-error")) {
        warning("Extreme-correction failed (", conditionMessage(last_err), "); keeping ML scores.")
        return(out_in)
      }
  ...
  # --- Post-hoc correction for extreme FS (parallel/mixed path) --------------
