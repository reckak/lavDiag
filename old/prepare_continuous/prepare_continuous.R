  ...
  for (g in seq_len(n_groups)) {
    ...
    for (j_idx in seq_along(ov_cont)) {
      ...
      for (k_idx in nz_idx) {
        ...
        eta_k  <- eta_mat[, k_idx]
        mu_vec <- as.numeric(nu_g[j_idx] + lam_row[k_idx] * eta_k)
        ...
        if (length(ids)) {
          uniq <- unique(ids); Vsub <- V[uniq, uniq, drop = FALSE]
          # gradient w.r.t. (nu_j, lambda_jk) is (1, eta_k)
          A    <- cbind(if (id_nu  > 0L) 1 else NULL,
                        if (id_lam > 0L) eta_k else NULL)
          cols <- c(if (id_nu  > 0L) id_nu  else NULL,
                    if (id_lam > 0L) id_lam else NULL)
          AV   <- A[, match(cols, uniq), drop = FALSE] %*% Vsub[match(cols, uniq), , drop = FALSE]
          se   <- sqrt(pmax(rowSums(AV * A[, match(uniq, cols), drop = FALSE]), 0))
        } else {
