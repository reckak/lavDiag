      ...
      function(j) {
        ...
        if (nrow(X_fit) >= length(rel_lats) + 2L) {
          ...
          if (j %in% ov_cont) {
            ...
            mu  <- to_original_scale(clamp01(pred_df[[1L]]))
            lwr <- to_original_scale(clamp01(pred_df[[2L]]))
            upr <- to_original_scale(clamp01(pred_df[[3L]]))
          }
