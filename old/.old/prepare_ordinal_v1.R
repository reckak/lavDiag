    ...
    for (jj in seq_along(ov_ord)) {
      ...
      for (k_idx in nz_idx) {
        ...
        get_id_nu <- function(g, jname) {
          # for ordinals, ν_j is typically fixed to 0; if not free, ID stays 0
          ix <- which(coef_names == paste0(jname, "~1"))
          if (!length(ix)) 0L else ix[1L]
        }
        get_id_lam <- function(g, jname, kname) {
          # search by "=~" label in parTable-style names; fallback by regex
          nm <- paste0(kname, "=~", jname)
          ix <- which(coef_names == nm)
          if (length(ix)) return(ix[1L])
          # alternative names (safety)
          ix <- grep(paste0("^", kname, "=~", jname, "$"), coef_names)
          if (length(ix)) ix[1L] else 0L
        }
        get_id_tau <- function(g, jname, m) {
          # lavaan uses names like "j|t1", "j|t2", ...
          nm <- paste0(jname, "|t", seq_len(m-1L))
          vapply(nm, function(s) {
            ix <- which(coef_names == s)
            if (length(ix)) ix[1L] else 0L
          }, integer(1))
        }
