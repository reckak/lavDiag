    ...
    for (i in seq_along(nz_idx)) {
      ...
      if (!compute_se) {
        ...
        if (length(A_ids)) {
          ...
          seen <- rep(FALSE, p)
          map_idx <- match(A_ids, uniq)
          col_ptr <- 1L
          for (mm in seq_along(A_mats)) {
            M <- A_mats[[mm]]
            if (!is.null(M) && ncol(M) > 0L) {
              for (jjj in seq_len(ncol(M))) {
                tgt <- map_idx[col_ptr]
                if (!seen[tgt]) {
                  A[, tgt] <- M[, jjj]
                  seen[tgt] <- TRUE
          ...
          if (inherits(L, "try-error")) {
            ev <- eigen(Vsub, symmetric = TRUE)
            vals <- pmax(ev$values, 0)
            if (all(vals == 0)) {
              se_mu <- rep(0, n)
            } else {
              L_eig <- ev$vectors %*% diag(sqrt(vals), nrow = length(vals))
              Bc <- A %*% L_eig
