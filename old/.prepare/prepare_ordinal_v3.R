...
prepare_ordinal_old <- function(fit,
  ...
  fs_df <- if (is.null(data)) {
    lavPredict_parallel(fit, return_type = "data", se = compute_se, prefix_se = prefix_se)
  } else data
  ...
  for (g in seq_len(n_groups)) {
    ...
    for (k in seq_along(latent_names)) {
      ...
      # allocate grid row-wise without rep()
  ...
  # If SEs are requested, fetch vcov + parTable; otherwise skip entirely
  if (compute_se) {
  ...
  # Baseline means for non-focal latents per group (used in compute_one)
  ...
  # Parameter-ID mapping only if SEs are needed
  id_map <- NULL
  if (compute_se) {
  ...
  # Helper to fetch thresholds per item quickly (restored)
  get_tau_for <- function(th_vec, j) {
    # Comments in English: fixed escaping of '|' and backreference in gsub
  ...
  compute_one <- function(g, jj) {
    ...
    # constant part of y* common to all k
    ystar0 <- nu_j + sum(lam_row[nz_idx] * base_eta[nz_idx])
    ...
    for (i in seq_along(nz_idx)) {
      ...
      # y* using shared constant
      ystar <- ystar0 + lam_row[k_idx] * (eta_k - base_eta[k_name])
      ...
      if (!compute_se) {
        ...
        # Delta-method pieces only when SEs are requested
        ...
        if (length(A_ids)) {
          ...
          L <- try(chol(Vsub), silent = TRUE)
          if (inherits(L, "try-error")) {
            se_mu <- sqrt(pmax(rowSums((A %*% Vsub) * A), 0))
          } else {
            Bc <- A %*% L
            se_mu <- sqrt(pmax(rowSums(Bc * Bc), 0))
          }
        } else {
      ...
      if (compute_se) {
        ...
        out_parts[[i]] <- list(rows = rows_k, nm_e = nm_e, nm_l = nm_l, nm_u = nm_u,
                               est = mu_vec, lwr = rep(NA_real_, length(rows_k)), upr = rep(NA_real_, length(rows_k)))
      }
    }
