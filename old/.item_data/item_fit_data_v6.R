item_fit_data_old <- function(fit,
  ...
  # If user explicitly passes betar() for continuous branch, check mgcv version
  ...
  for (g in seq_len(n_groups)) {
    ...
    fml_cache <- new.env(parent = emptyenv())
    newdata_cache <- new.env(parent = emptyenv())
        ...
        for (j in ov_all) {
          ...
          for (k in unique(lat_in_row)) {
            ...
            if (is_ocat && (j %in% ov_ord)) {
              ...
              if ((j %in% ov_ord && !is_ocat) || (is.list(fit_gj$family) && !is.null(fit_gj$family$family) && identical(fit_gj$family$family, "Beta regression"))) {
                rj <- rng_by_item[[j]]
                to_original_scale <- function(p01) { d <- rj[2] - rj[1]; p01 * d + rj[1] }
                cn <- names(pred_df)
