...
#' Additionally returns a per-item/per-group metrics tibble with R^2, RMSE, MAE,
#' and complexity-penalized variants.
#'
...
#'
#' Metrics: For each item and group, the function computes R^2 (from Pearson r^2),
#' RMSE, and MAE between `m_est_yhat_*` and `e_est_yhat_*` on the original scale.
#' Penalized variants use an effective complexity
#' `k_eff = 2 + c_m + c_e`, where `c_m` is model-side complexity
#' (number of relevant latents + intercept for continuous items, or **number of free thresholds** for ordinal items)
#' and `c_e` is empirical complexity (sum of edf of the fitted GAM).
#' Penalization formulas:
#' `R2_pen = 1 - (1 - R2) * (n_eff - 1) / pmax(1, n_eff - k_eff - 1)`,
#' `RMSE_pen = sqrt(MSE * n_eff / pmax(1, n_eff - k_eff))`,
#' and `MAE_pen = MAE * sqrt(n_eff / pmax(1, n_eff - k_eff))` (heuristic scaling).
#'
#' @return A list with two tibbles:
#'   - `$data`: original augmented data with `m_*` and `e_*` columns (as before),
...
prepare <- function(fit,
  ...
  thr_counts_by_group <- lapply(split(PE_thr, PE_thr$group), function(PEg) {
    ...
    # count ALL thresholds (handles constraints naturally)
    tabs_all <- tapply(PEg$lhs, PEg$lhs, length)
    # count FREE thresholds (unique 'free' ids > 0)
      ...
      function(j) {
        ...
        if (!nrow(df_ok)) {
          ...
          return(list(
            ...
            lwr = rep(NA_real_, n),
            upr = rep(NA_real_, n),
            ce  = NA_real_   # empirical complexity (edf sum)
          ))
        }
        ...
        if (nrow(X_fit) >= length(rel_lats) + 2L) {
          ...
          ce  <- sum(fit_gam$edf, na.rm = TRUE)  # empirical complexity
        } else {
          n <- nrow(df_g)
  ...
  for (g in seq_len(n_groups)) {
    ...
    g_label <- if (n_groups > 1L) {
      ...
      if (gvar %in% names(out)) {
        ...
        # try model_info labels if available
    ...
    for (j in ov_all) {
      ...
      # model-side complexity c_m: relevant latents + (intercept or free thresholds)
      # pick rel_lats the same way as in fitting (respect vary_by_group)
      rel_lats <- if (vary_by_group) {
        li_g <- loads_by_item_by_group[[as.character(g)]] %||% loads_by_item_by_group[[g]]
        (if (!is.null(li_g)) li_g[[j]] else NULL) %||% loads_by_item_overall[[j]]
      } else {
      ...
      if (j %in% ov_cont) {
        ...
        # prefer ALL thresholds (K-1); fallback na FREE a úplný fallback na data/levels
        ...
        n_thr <- n_thr_all
        if (!is.finite(n_thr) || n_thr <= 0L) n_thr <- n_thr_free

        if (!is.finite(n_thr) || n_thr <= 0L) {
          # last-resort: infer from data (levels) if possible
          xj <- out[[j]]
      ...
      ce <- ce_vec[[j]]
      k_eff <- 2 + cm + (if (is.finite(ce)) ce else 0)  # 2 = intercept+slope in calibration e~m
      ...
      metrics[[length(metrics) + 1L]] <- list(
        ...
        k_eff     = as.numeric(if (is.finite(ce)) 2 + cm + ce else 2 + cm),
        r2        = r2,
        rmse      = rmse,
        mae       = mae,
        r2_pen    = r2_pen,
