...
#' to all available factor-score columns.
#'
...
prepare_original_old <- function(fit,
  ...
  loads_by_item_by_group <- lapply(PE_by_group, function(PEg) {
    ...
    # Ensure presence of all observed items as keys
  ...
  for (g in seq_len(n_groups)) {
    ...
    item_results <- furrr::future_map(
      ...
      function(j) {
        ...
        rel_lats <- if (!is.null(li_g)) li_g[[j]] else NULL
        if (is.null(rel_lats) || !length(rel_lats)) rel_lats <- loads_by_item_overall[[j]]
        if (is.null(rel_lats) || !length(rel_lats)) rel_lats <- eta_cols
        rel_lats <- intersect(rel_lats, eta_cols)
        ...
        # Formula with additive smooths per relevant latent
