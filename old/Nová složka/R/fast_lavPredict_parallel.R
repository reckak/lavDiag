...
lavPredict_parallel_old <- function(fit,
  ...
  # Keep only ordinal variables that truly exist in data
  ...
  # -------- Driver data (what we actually predict on) -------------------------
  # For purely ordinal models we may shrink work via distinct on FULL rows (safe).
  # For mixed models we MUST keep full rows (predictions depend on continuous values).
  use_distinct <- switch(distinct,
                         "always" = TRUE,
                         "never"  = FALSE,
                         "auto"   = (nrow(dat_original) >= distinct_threshold))
  if (has_cont) {
    # Disable distinct for mixed models to keep dependence on continuous vars
    use_distinct <- FALSE
  }

  dat_driver <- if (use_distinct) dplyr::distinct(dat_original) else dat_original

  # -------- Build dummy rows (ordinal categories + continuous variance) -------
  ...
  dummy <- dummy[, intersect(names(dummy), names(dat_original)), drop = FALSE]
  ...
  # Prepend dummy (once per chunk; guarantees categories + variance per group)
  if (nrow(dummy) > 0L) {
    chunked <- lapply(chunked, function(x) dplyr::bind_rows(dummy, x))
  }

  # -------- Plan set/reset ----------------------------------------------------
  reset_plan <- .set_future_plan(plan = plan, workers = workers)  # from set_future_plan.txt
  ...
  # Merge back
  if (use_distinct) {
    # We distinct()'d on full rows, so a simple left_join on all shared columns is ok.
    ...
    # Rely on preserved row order (chunks created from full dat_driver in order)
    if (nrow(out_all) != nrow(dat_driver)) {
      stop("lavPredict_parallel: row count mismatch in non-distinct path.", call. = FALSE)
    }
    pred_cols   <- setdiff(names(out_all), names(dat_driver))
    pred_mat    <- out_all[, pred_cols, drop = FALSE]
    # If driver == original, bind directly; otherwise, re-map by matching rows
    if (identical(nrow(dat_driver), nrow(dat_original)) &&
        identical(names(dat_driver), names(dat_original))) {
      out_joined <- dplyr::bind_cols(dat_original, pred_mat)
    } else {
      # Conservative fallback: join on all original columns that survive through lavaan
      join_keys <- intersect(names(dat_driver), names(out_all))
      out_joined <- dplyr::left_join(dat_original, out_all, by = join_keys)
    }
  }
